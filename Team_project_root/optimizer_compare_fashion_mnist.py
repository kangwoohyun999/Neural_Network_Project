...
# ====== (추가) Loss 곡선 부드럽게 만드는 함수 ======
def smooth_curve(values, window_size=50):
    """
    단순 이동 평균으로 값을 부드럽게 만드는 함수.
    values: 1차원 리스트/배열
    window_size: 몇 개 값씩 평균 낼지 (기본 50)
    """
    values = np.array(values, dtype=np.float32)
    if len(values) < window_size:
        return values  # 데이터가 너무 짧으면 그대로 반환
...
def main():
    ...
    # 6. Loss 곡선 그래프 그리기 (스무딩 적용)
    plt.figure(figsize=(8, 5))
    iters = np.arange(max_iterations)
    window = 50  # 50 step 단위로 이동 평균

    for key in optimizers.keys():
        loss_list = train_loss_history[key]
        smooth_loss = smooth_curve(loss_list, window_size=window)
        smooth_iters = iters[window-1:]  # 길이에 맞춰서 x축도 잘라줌
        plt.plot(smooth_iters, smooth_loss, label=key)

    plt.xlabel("Iterations")
    plt.ylabel("Loss")
    plt.title("Optimizer Comparison on Fashion-MNIST (Smoothed Train Loss)")
    plt.legend()
    plt.grid(True)
    plt.ylim(0.0, 1.0)  # 필요에 따라 범위 조절
    plt.tight_layout()
    plt.savefig("optimizer_compare_loss.png")
    print("\nLoss 비교 그래프가 'optimizer_compare_loss.png' 이름으로 저장되었습니다.")

    # 7. Test Accuracy 막대 그래프
